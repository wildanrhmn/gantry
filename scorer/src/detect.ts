          ...
          const victims = ordered
            .filter(
              (s) =>
                s.index > front.index &&
                s.index < back.index &&
                s.sender.toLowerCase() !== address &&
                s.zeroForOne === front.zeroForOne,
            )
            .map((s) => s.sender);
...
export function buildFeatures(swaps: Swap[], sandwiches = detectSandwiches(swaps)): AddressFeatures[] {
  ...
  const acc = new Map<string, AddressFeatures & { blockSet: Set<number> }>();
  ...
  return [...acc.values()].map(({ blockSet, ...rest }) => ({ ...rest, blocks: blockSet.size }));
}
